...
struct LIGHT
{
	...
	BOOL Dummy[3];
	DirectX::SimpleMath::Vector4 Direction;//平行光源の方向に
	DirectX::SimpleMath::Color Diffuse;	//平行の強さと色
...
//サブセット
struct SUBSET {
	std::string MtrlName;//マテリアル
	unsigned	int IndexNum = 0;//頂点ベース
	unsigned	int VertexNum = 0;//インデックス数
	unsigned	int	IndexBase = 0;//頂数
	unsigned	int VertexBase = 0;//開始インデックス
